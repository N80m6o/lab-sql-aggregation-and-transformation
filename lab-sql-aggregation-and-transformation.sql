...
select max(rental_date) from rental as EndDate;
select min(rental_date) from rental as StartDate;

select datediff(max(rental_date), min(rental_date)) as Days_of_Operations from rental;
